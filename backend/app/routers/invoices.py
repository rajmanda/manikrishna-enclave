...
async def create_invoice(body: InvoiceCreate, db: DB, user: CurrentUser) -> Invoice:
    ...
    await enqueue_for_apartment_owners(
        ...
        title="New Invoice",
        message=f"Sent by {user.name}. Invoice {invoice.description}: Rs {body.amount:,.0f} due {body.due_date}. View details: https://community.rajmanda.com/invoices",
    ...
    if created > 0:
        ...
        for apt in apts_for_notif:
            await enqueue_for_apartment_owners(
                ...
                event_type="invoice_created",
                title="New Invoice",
                message=f"Sent by {user.name}. {body.description} — {body.period}: Rs {amount:,.0f} due {body.due_date}. View details: https://community.rajmanda.com/invoices",
...
async def bill_owner(body: BillOwnerRequest, db: DB, user: CurrentUser) -> Invoice:
    ...
    await enqueue_for_apartment_owners(
        ...
        event_type="invoice_created",
        title="Itemized Bill",
        message=f"Billed by {user.name}. Itemized charge Rs {total:,.0f} for {invoice.description}. View details: https://community.rajmanda.com/invoices",
    ...
    if payment.get("reported_by"):
        ...
        if reporter and reporter.get("phone"):
            await enqueue_notification(
                ...
                message=f"Confirmed by {user.name}. Your payment of Rs {payment['amount']:,.0f} has been confirmed. View details: https://community.rajmanda.com/invoices",
