...
async def create_work_order(
    ...
    # One WhatsApp message to the community group chat.
    await enqueue_for_community_group(
        db,
        community_id=user.community_id,
        event_type="work_order_created",
        title="New Work Order",
        message=f"🛠️ New work order by {user.display_name}: {wo.title} (Priority: {wo.priority}). View details: {app_url()}/work-orders/{wo.id}",
...
async def change_stage(
    ...
    # One WhatsApp message to the community group chat; Owner Approval gets
    # its own wording since it asks the owners to act.
    event_type = "work_order_status_updated"
    title = "Work Order Update"
    message = f"🛠️ Work order '{wo['title']}' moved to {body.stage} (by {user.display_name}). View details: {app_url()}/work-orders/{work_order_id}"
    if body.stage == "Owner Approval":
        event_type = "owner_approval_required"
        title = "Approval Required"
        message = f"⚠️ Work order '{wo['title']}' needs owner approval (requested by {user.display_name}). Review here: {app_url()}/work-orders/{work_order_id}"
    await enqueue_for_community_group(
        db,
        community_id=user.community_id,
